...
describe('parseHeader', () => {
  ...
  test('handles leading whitespace before the key', () => {
    expect(parseHeader(' \tTheme Name: My Theme', 'Theme Name')).toBe('My Theme');
  });
  test('handles extra spaces around the colon', () => {
    expect(parseHeader('Theme Name : Spacey Theme', 'Theme Name')).toBe('Spacey Theme');
  });
  test('returns null when the key is absent', () => {
    expect(parseHeader('/* nothing useful here */', 'Theme Name')).toBeNull();
  });
  test('trims trailing whitespace from the value', () => {
    expect(parseHeader('Theme Name: Clean   ', 'Theme Name')).toBe('Clean');
  });
  test('parses PHPDoc-style header with leading asterisk (WordPress.org format)', () => {
    expect(parseHeader(' * Plugin Name:       Advanced Custom Fields', 'Plugin Name')).toBe('Advanced Custom Fields');
  });
});
...
describe('isWxr', () => {
  ...
  test('returns false when rss marker is absent', () => {
    expect(isWxr('<?xml?>\n<feed xmlns:excerpt="http://wordpress.org/export/">')).toBe(false);
  });
  test('returns false when xmlns:excerpt marker is absent', () => {
    expect(isWxr('<?xml?>\n<rss version="2.0">')).toBe(false);
  });
